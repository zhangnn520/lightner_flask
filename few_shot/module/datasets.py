...
class ConllNERProcessor(object):
    def __init__(self, lines=None, mapping=None, bart_name=None,learn_weights=None) -> None:
        self.lines = lines
    ...
    def process(self, data_dict):
        ...
        def prepare_target(item):
            ...
            for word in raw_word:
                bpes = self.tokenizer.tokenize(word) if 'chinese' in self.bart_name else self.tokenizer.tokenize(word,
            ...
            word_bpes.append(
                [self.tokenizer.sep_token_id if 'chinese' in self.bart_name else self.tokenizer.eos_token_id])
        ...
        for raw_word, raw_target, entity, entity_tag, entity_span in tqdm(
                zip(data_dict['raw_words'], data_dict['raw_targets'], data_dict['entities'],
                    data_dict['entity_tags'], data_dict['entity_spans']), total=len(data_dict['raw_words']),
                desc='Processing'):
            item_dict = prepare_target(
                {'raw_word': raw_word, 'raw_target': raw_target, 'entity': entity, 'entity_tag': entity_tag,
        ...
        if self.learn_weights:  # add extra tokens to huggingface tokenizer
            ...
            for key, value in self.mapping.items():
                key_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(value[2:-2]) if 'chinese' in
                                                                                                      self.bart_name else self.tokenizer.tokenize(
                    value[2:-2], add_prefix_space=True))
...
class ConllNERDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'test':
            return torch.tensor(self.complet_data['src_tokens'][index]), torch.tensor(
                self.complet_data['src_seq_len'][index]), \
                   torch.tensor(self.complet_data['first'][index]), self.complet_data['raw_words'][index]

        return torch.tensor(self.complet_data['src_tokens'][index]), torch.tensor(
            self.complet_data['tgt_tokens'][index]), \
               torch.tensor(self.complet_data['src_seq_len'][index]), torch.tensor(
            self.complet_data['tgt_seq_len'][index]), \
    ...
    def collate_fn(self, batch):
        ...
        if self.mode == "test":
            ...
            src_tokens = pad_sequence(src_tokens, batch_first=True,
                                      padding_value=self.data_processor.tokenizer.pad_token_id)
...
# if __name__ == '__main__':
    # data_path = {'train': 'data/conll2003/train.json'}
